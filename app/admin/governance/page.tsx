...
export default async function Page(){
 ...
 const [corrections,deletions,rejectedReports,rejectedMembers,holds]=await Promise.all([
  ...
  db.prepare("SELECT m.id,m.company_name,m.created_at base,EXISTS(SELECT 1 FROM legal_holds h WHERE h.target_type='member' AND h.target_id=CAST(m.id AS TEXT) AND h.released_at IS NULL) held FROM members m WHERE m.status='rejected' AND m.created_at<?").bind(warning).all<any>(),
